...
- Follow the project's document ID convention when one exists. Otherwise identify requirements as `{FEATURE}-RQ-{NNN}`; for a platform-specific requirement, use `{FEATURE}-{PLATFORM}-RQ-{NNN}`. Retain the same IDs in related functional specifications, screen designs, and test scenarios. Platform abbreviations are project-defined.
